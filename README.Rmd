---
title: "Lab_README_File"
author: "Chi Chi Okezie"
date: "2025-08-12"
...
**I think that the Fynbos greenness (NDVI) as observed from the satellite saturates, leveling off around before reaching 0.7 as a reflection of the limitations of utilizing NDVI and it is also indicative of the nature of Fynbos. As we talked about in class, the NDVI index measures the difference between near-infrared and red light to build a ratio that classifies land cover on scale from 0-1.0. Near-infrared light is reflected with higher chlorophyll content detailing the health and “greenness” of the vegetation. As Fynbos plants vary in colors they're not always going to be "green" in the way that would be picked up and measured by the satellite so at some point once most of the land is covered and thus most of the ground is covered by the canopy whatever is the total measure of chlorophyll content from the canopy cover will reflect the greenness of the very surface of the vegetation and this doesn't change much once established. Thus,  failing to account for the continuously changing ground level of the plants. By using these units,canopy cover and NDVI, does not allow for real encapsulation of all of the development that is happening within the plants over this time. Thus, once the satellite  cannot detect changes in the canopy cover of the plants, the NDVI levels, even despite changes on the ground level that we cannot see.  ** 
...
**The two models share the common parameters: alpha, gamma, and lambda. From the model summary the estimates of alpha and gamma are both within 0.05 of each other, with lambda having the most significant difference of ~0.19. This tells us that rate of recovery estimate is sensitive/variable based on whether you account for seasonal oscillations or not.** 
...
**Based on the ANOVA results, the three stars next to model 2 indicate including seasonality is distinct enough to include this as an additional parameter as this adds new and relevant information to the simpler model, explaining more of the patterns in the data.**

> Question 4: How many degrees of freedom are there in this ANOVA and why (i.e. what are they)? (3 marks)

<br>

**There are 2 degrees of freedom as degrees of freedom refers to the the difference in the number of parameters between the models. Model one has parameters alpha, gamma, and lambda and model two has parameters alpha, gamma, lambda, A and phi, thus parameters A and phi represent the two degrees of freedom that the ANOVA references.**
...
**The lower AIC value of 451159.3 (AIC_full / AIC of the full model)indicates that this would be the 'better' model. The difference between the simple and full models (AIC_simple - AIC_full) being 266953.8 would also indicate that these models are significantly different.**
...
**As mentioned within the R documentation, the theory of Akaike's information criterion (AIC) requires that the log-likelihood has been maximized, and while the AIC can be computed for models not fitted by maximum likelihood, their AIC values should not be compared because they are based on different estimation criteria. Using the example of non-linear least squares (NLS) models and comparing this with Maximum Likelihood Estimation (MLE) models, NLS minimizes sum of squares, not likelihood, so the scales of the AIC values will be different, and thus not sensibly comparable. **
...
**The posterior density plots represents our uncertainty (or certainty) in each of our parameters, after combining the information in the data with the prior. The the higher the mass of the distribution in one place in a plot is representative of greater certainty but the mass being more spread out indicates a greater uncertainty in the represented parameter.**
...
**No, looking at the distributions of the posterior estimates, the distributions are primarily concentrated in a certain area for each parameter, indicative of a greater certainty in these parameter estimates.**
<br>
...
**These are single-level Bayesian models, a single-level Bayes model use Bayes' theorem to combine prior distributions with observed data to derive the posterior distribution. Whereas a hierarchical Bayes model considers different levels of parameters, where parameters at each level follow their own distributions, described by hyperparameters, indicated within the code. Looking to our model, with brm() we fix all parameters (alpha, gamma, lambda, A, phi) to ~ 1 which indicates that there are not differing levels to the parameters, thus it cannot be a hierarchical model.**
