...
**I think that the Fynbos greenness (NDVI) as observed from the
satellite saturates, leveling off around before reaching 0.7 as a
reflection of the limitations of utilizing NDVI and it is also
indicative of the nature of Fynbos. As we talked about in class, the
NDVI index measures the difference between near-infrared and red light
to build a ratio that classifies land cover on scale from 0-1.0.
Near-infrared light is reflected with higher chlorophyll content
detailing the health and “greenness” of the vegetation. As Fynbos plants
vary in colors they’re not always going to be “green” in the way that
would be picked up and measured by the satellite so at some point once
most of the land is covered and thus most of the ground is covered by
the canopy whatever is the total measure of chlorophyll content from the
canopy cover will reflect the greenness of the very surface of the
vegetation and this doesn’t change much once established. Thus, failing
to account for the continuously changing ground level of the plants. By
using these units,canopy cover and NDVI, does not allow for real
encapsulation of all of the development that is happening within the
plants over this time. Thus, once the satellite cannot detect changes in
the canopy cover of the plants, the NDVI levels, even despite changes on
the ground level that we cannot see.**
...
**The two models share the common parameters: alpha, gamma, and lambda.
From the model summary the estimates of alpha and gamma are both within
0.05 of each other, with lambda having the most significant difference
of ~0.19. This tells us that rate of recovery estimate is
sensitive/variable based on whether you account for seasonal
oscillations or not.**
...
**Based on the ANOVA results, the three stars next to model 2 indicate
including seasonality is distinct enough to include this as an
additional parameter as this adds new and relevant information to the
simpler model, explaining more of the patterns in the data.**
...
**There are 2 degrees of freedom as degrees of freedom refers to the the
difference in the number of parameters between the models. Model one has
parameters alpha, gamma, and lambda and model two has parameters alpha,
gamma, lambda, A and phi, thus parameters A and phi represent the two
degrees of freedom that the ANOVA references.**
...
**The lower AIC value of 451159.3 (AIC_full / AIC of the full
model)indicates that this would be the ‘better’ model. The difference
between the simple and full models (AIC_simple - AIC_full) being
266953.8 would also indicate that these models are significantly
different.**
...
**As mentioned within the R documentation, the theory of Akaike’s
information criterion (AIC) requires that the log-likelihood has been
maximized, and while the AIC can be computed for models not fitted by
maximum likelihood, their AIC values should not be compared because they
are based on different estimation criteria. Using the example of
non-linear least squares (NLS) models and comparing this with Maximum
Likelihood Estimation (MLE) models, NLS minimizes sum of squares, not
likelihood, so the scales of the AIC values will be different, and thus
not sensibly comparable.**
...
```
    ...
    ## alpha_Intercept      0.25      0.03     0.19     0.31 1.00      935     1029
    ## gamma_Intercept      0.33      0.03     0.27     0.38 1.00     1008     1295
    ## lambda_Intercept     1.21      0.28     0.76     1.88 1.00     1250     1782
    ## 
    ## Further Distributional Parameters:
    ##       Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
    ## sigma     0.07      0.00     0.07     0.08 1.00     1358     1443
    ## 
    ...
    ## Chain 2: 
    ## Chain 2:  Elapsed Time: 0.433 seconds (Warm-up)
    ## Chain 2:                0.371 seconds (Sampling)
...
**The posterior density plots represents our uncertainty (or certainty)
in each of our parameters, after combining the information in the data
with the prior. The the higher the mass of the distribution in one place
in a plot is representative of greater certainty but the mass being more
spread out indicates a greater uncertainty in the represented
parameter.**
...
```
    ...
    ## Regression Coefficients:
    ##                  Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
    ## alpha_Intercept      0.21      0.03     0.15     0.26 1.00     1164     1275
    ## gamma_Intercept      0.36      0.03     0.32     0.42 1.00     1264     1473
    ## lambda_Intercept     1.02      0.18     0.73     1.42 1.00     1514     1864
    ## A_Intercept          0.06      0.01     0.05     0.08 1.00     1799     1685
    ## phi_Intercept       -0.84      0.11    -1.06    -0.62 1.00     2177     1822
    ## 
    ## Further Distributional Parameters:
    ##       Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
    ## sigma     0.06      0.00     0.05     0.07 1.00     2522     1914
    ## 
    ## Draws were sampled using sampling(NUTS). For each parameter, Bulk_ESS
    ## and Tail_ESS are effective sample size measures, and Rhat is the potential
    ## scale reduction factor on split chains (at convergence, Rhat = 1).
    ...
    ## Chain 3: Iteration: 2000 / 2000 [100%]  (Sampling)
    ## Chain 3: 
    ## Chain 3:  Elapsed Time: 1.022 seconds (Warm-up)
    ## Chain 3:                0.935 seconds (Sampling)
...
**No, looking at the distributions of the posterior estimates, the
distributions are primarily concentrated in a certain area for each
parameter, indicative of a greater certainty in these parameter
estimates.** <br>
...
**These are single-level Bayesian models, a single-level Bayes model use
Bayes’ theorem to combine prior distributions with observed data to
derive the posterior distribution. Whereas a hierarchical Bayes model
considers different levels of parameters, where parameters at each level
follow their own distributions, described by hyperparameters, indicated
within the code. Looking to our model, with brm() we fix all parameters
(alpha, gamma, lambda, A, phi) to ~ 1 which indicates that there are not
differing levels to the parameters, thus it cannot be a hierarchical
model.**
